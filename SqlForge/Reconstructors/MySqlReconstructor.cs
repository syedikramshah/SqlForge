...
namespace SqlForge.Reconstructors
{
    public class MySqlReconstructor : BaseSqlReconstructor
    {
        ...
        public override string Reconstruct(SqlStatement statement, SqlDialect dialect = SqlDialect.MySql)
        {
        ...
        private void ReconstructNode(ISqlNode node)
        {
            ...
            switch (node)
            {
                ...
                case CommonTableExpression cte:
                    _sb.Append(QuoteIdentifier(cte.Name, QuoteStyle.Backtick));
                    if (cte.ColumnNames.Any())
                    {
                        _sb.Append(" (");
                        for (int i = 0; i < cte.ColumnNames.Count; i++)
                        {
                            _sb.Append(QuoteIdentifier(cte.ColumnNames[i], QuoteStyle.Backtick));
                ...
                case SelectStatement select:
                    _sb.Append("SELECT ");
                    if (select.IsDistinctRow)
                    {
                        _sb.Append("DISTINCTROW ");
                    }
                    ...
                    if (select.SelectModifiers != null && select.SelectModifiers.Any())
                    {
                        foreach (var modifier in select.SelectModifiers)
                        {
                            _sb.Append(modifier + " ");
                        }
                    }
                    ...
                    {
                        _sb.Append(" ");
                        ReconstructNode(select.IntoClause);
                    }
                    ...
                    {
                        _sb.Append(" ");
                        ReconstructNode(select.LimitClause);
                    }

                    if (select.ForUpdate)
                    {
                        _sb.Append(" FOR UPDATE");
                    }

                    if (select.LockInShareMode)
                    {
                        _sb.Append(" LOCK IN SHARE MODE");
                    }
                    break;
                ...
                case LimitClause limit:
                    ...
                    {
                        ReconstructNode(limit.CountExpression);
                    }

                    if (limit.OffsetExpression != null)
                    {
                        _sb.Append(" OFFSET ");
                        ReconstructNode(limit.OffsetExpression);
                    }
                    break;

                case SelectIntoClause intoClause:
                    _sb.Append("INTO ");
                    _sb.Append(intoClause.Type == SelectIntoType.Outfile ? "OUTFILE " : "DUMPFILE ");
                    _sb.Append($"'{intoClause.FilePath.Replace("'", "''")}'");
                    break;
                ...
                case BinaryExpression bin:
                    bool leftParens = NeedsParenForPrecedence(bin.Operator, bin.Left);
                    bool rightParens = NeedsParenForPrecedence(bin.Operator, bin.Right);

                    if (leftParens) _sb.Append("(");
                    ReconstructNode(bin.Left);
                    if (leftParens) _sb.Append(")");

                    _sb.Append(" " + bin.Operator + " ");

                    if (rightParens) _sb.Append("(");
                    ReconstructNode(bin.Right);
                    if (rightParens) _sb.Append(")");
                    break;

                case BetweenExpression between:
                    ReconstructNode(between.Expression);
                    _sb.Append(between.IsNegated ? " NOT BETWEEN " : " BETWEEN ");
                    ReconstructNode(between.Lower);
                    _sb.Append(" AND ");
                    ReconstructNode(between.Upper);
                    break;
                ...
                case InsertStatement insert:
                    _sb.Append(insert.IsReplace ? "REPLACE" : "INSERT");
                    if (insert.IsIgnore)
                    {
                        _sb.Append(" IGNORE");
                    }
                    ...
                    if (insert.SetAssignments.Any())
                    {
                        _sb.Append(" SET ");
                        for (int i = 0; i < insert.SetAssignments.Count; i++)
                        {
                            ReconstructNode(insert.SetAssignments[i]);
                            if (i < insert.SetAssignments.Count - 1)
                            {
                                _sb.Append(", ");
                            }
                        }
                    }
                    else if (insert.Values.Any())
                    {
                        _sb.Append(" VALUES ");
                        for (int rowIndex = 0; rowIndex < insert.Values.Count; rowIndex++)
                        {
                            _sb.Append("(");
                            var row = insert.Values[rowIndex];
                            for (int i = 0; i < row.Count; i++)
                            {
                                ReconstructNode(row[i]);
                                if (i < row.Count - 1)
                                {
                                    _sb.Append(", ");
                                }
                            ...
                            _sb.Append(")");
                            if (rowIndex < insert.Values.Count - 1)
                            {
                                _sb.Append(", ");
                            }
                        }
                    ...
                    if (insert.OnDuplicateKeyUpdate.Any())
                    {
                        _sb.Append(" ON DUPLICATE KEY UPDATE ");
                        for (int i = 0; i < insert.OnDuplicateKeyUpdate.Count; i++)
                        {
                            ReconstructNode(insert.OnDuplicateKeyUpdate[i]);
                            if (i < insert.OnDuplicateKeyUpdate.Count - 1)
                            {
                                _sb.Append(", ");
                            }
                        }
                ...
                case UpdateStatement update:
                    _sb.Append("UPDATE");
                    if (update.IsLowPriority)
                    {
                        _sb.Append(" LOW_PRIORITY");
                    }
                    if (update.IsIgnore)
                    {
                        _sb.Append(" IGNORE");
                    ...
                    if (update.OrderByClause != null)
                    {
                        _sb.Append(" ORDER BY ");
                        ReconstructNode(update.OrderByClause);
                    }
                    if (update.LimitClause != null)
                    {
                        _sb.Append(" ");
                        ReconstructNode(update.LimitClause);
                    }
                    break;

                case DeleteStatement delete:
                    _sb.Append("DELETE");
                    if (delete.IsLowPriority)
                    {
                        _sb.Append(" LOW_PRIORITY");
                    }
                    if (delete.IsQuick)
                    ...
                    if (delete.OrderByClause != null)
                    {
                        _sb.Append(" ORDER BY ");
                        ReconstructNode(delete.OrderByClause);
                    }
                    if (delete.LimitClause != null)
                    {
                        _sb.Append(" ");
                        ReconstructNode(delete.LimitClause);
                    }
                    break;
                ...
                case CreateTableStatement create:
                    ...
                    {
                        _sb.Append(" TEMPORARY");
                    }
                    _sb.Append(" TABLE");
                    if (create.IfNotExists)
                    {
                        _sb.Append(" IF NOT EXISTS");
                    }
                    _sb.Append(" ");
                    ReconstructNode(create.Target);
                    if (create.Columns.Any() || create.Constraints.Any() || create.Indexes.Any())
                    {
                        _sb.Append(" (");
                        bool first = true;
                        ...
                        {
                            if (!first) _sb.Append(", ");
                            ReconstructNode(constraint);
                            first = false;
                        }
                        foreach (var index in create.Indexes)
                        {
                            if (!first) _sb.Append(", ");
                            ReconstructNode(index);
                            first = false;
                        }
                        _sb.Append(")");
                    }
                    if (create.TableOptions.Any())
                    {
                        _sb.Append(" ");
                        for (int i = 0; i < create.TableOptions.Count; i++)
                        {
                            ReconstructNode(create.TableOptions[i]);
                            if (i < create.TableOptions.Count - 1)
                            {
                                _sb.Append(" ");
                            }
                        }
                ...
                case CreateIndexStatement createIndex:
                    ...
                    else if (createIndex.Index.Type == IndexType.Fulltext)
                    {
                        _sb.Append("FULLTEXT ");
                    }
                    else if (createIndex.Index.Type == IndexType.Spatial)
                    {
                        _sb.Append("SPATIAL ");
                    }
                    ...
                    if (!string.IsNullOrEmpty(createIndex.Index.UsingType))
                    {
                        _sb.Append(" USING ");
                        _sb.Append(createIndex.Index.UsingType);
                    }

                    _sb.Append(" (");
                    for (int i = 0; i < createIndex.Index.Columns.Count; i++)
                    {
                        ReconstructNode(createIndex.Index.Columns[i]);
                        if (i < createIndex.Index.Columns.Count - 1)
                        {
                            _sb.Append(", ");
                        }
                    }
                ...
                case AlterTableAction action:
                    switch (action.ActionType)
                    {
                        ...
                        case AlterTableActionType.DropColumn:
                            ...
                            break;
                        case AlterTableActionType.ModifyColumn:
                            _sb.Append("MODIFY COLUMN ");
                            ReconstructNode(action.Column);
                            break;
                        case AlterTableActionType.AddIndex:
                            _sb.Append("ADD ");
                            ReconstructNode(action.Index);
                            break;
                        case AlterTableActionType.DropIndex:
                            if (action.DropPrimaryKey)
                            {
                                _sb.Append("DROP PRIMARY KEY");
                            }
                            else
                            {
                                _sb.Append("DROP INDEX ");
                                _sb.Append(QuoteIdentifier(action.IndexName, QuoteStyle.Backtick));
                            }
                            break;
                        ...
                        case AlterTableActionType.RenameColumn:
                            ...
                            break;
                        case AlterTableActionType.ChangeColumn:
                            _sb.Append("CHANGE COLUMN ");
                            _sb.Append(QuoteIdentifier(action.ColumnName, action.ColumnNameQuoteStyle, action.ColumnNameQuoted));
                            _sb.Append(" ");
                            ReconstructNode(action.Column);
                            break;
                ...
                case ColumnDefinition column:
                    ...
                    {
                        _sb.Append(" CHARACTER SET ");
                        _sb.Append(column.CharacterSet);
                    }
                    if (!string.IsNullOrEmpty(column.Collation))
                    {
                        _sb.Append(" COLLATE ");
                        _sb.Append(column.Collation);
                    }
                    ...
                    {
                        _sb.Append(" COMMENT '");
                        _sb.Append(column.Comment.Replace("'", "''"));
                        _sb.Append("'");
                    }
                    break;
                ...
                case TableConstraint constraint:
                    ...
                    {
                        ReconstructNode(constraint.Columns[i]);
                ...
                case IndexDefinition index:
                    switch (index.Type)
                    {
                        ...
                        case IndexType.Unique:
                            _sb.Append("UNIQUE KEY");
                            break;
                        case IndexType.Fulltext:
                            _sb.Append("FULLTEXT KEY");
                            break;
                        case IndexType.Spatial:
                            _sb.Append("SPATIAL KEY");
                            break;
                        default:
                            _sb.Append("KEY");
                            break;
                    ...
                    {
                        _sb.Append(" ");
                        _sb.Append(QuoteIdentifier(index.Name, QuoteStyle.Backtick));
                    }

                    if (!string.IsNullOrEmpty(index.UsingType))
                    {
                        _sb.Append(" USING ");
                        _sb.Append(index.UsingType);
                    }

                    _sb.Append(" (");
                    for (int i = 0; i < index.Columns.Count; i++)
                    {
                        ReconstructNode(index.Columns[i]);
                ...
                case IndexColumn indexColumn:
                    ...
                    if (indexColumn.LengthExpression != null)
                    {
                        _sb.Append("(");
                        ReconstructNode(indexColumn.LengthExpression);
                        _sb.Append(")");
                    }
                ...
                case TableOption option:
                    ...
                    if (!string.IsNullOrEmpty(option.Value))
                    {
                        ...
                        {
                            _sb.Append("'");
                            _sb.Append(option.Value.Replace("'", "''"));
                            _sb.Append("'");
                        }
                        else
                        {
                            _sb.Append(option.Value);
                        }
                    }
                    break;
        ...
        private static string QuoteIdentifier(string identifier, QuoteStyle quoteStyle, bool quotedFallback = false)
        {
